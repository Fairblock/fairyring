...
export { AggregatedKeyShare, AuthorizedAddress, Commitments, KeyShare, KeysharePacketData, NoData, RequestAggrKeysharePacketData, RequestAggrKeysharePacketAck, GetAggrKeysharePacketData, GetAggrKeysharePacketAck, AggrKeyshareDataPacketData, AggrKeyshareDataPacketAck, Params, ActivePubKey, QueuedPubKey, ValidatorSet };
...
export default {
	...
	actions: {
		...
		async sendMsgSendAggrKeyshareData({ rootGetters }, { value, fee = {amount: [], gas: "200000"}, memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const fullFee = Array.isArray(fee)  ? {amount: fee, gas: "200000"} :fee;
				const result = await client.FairyringKeyshare.tx.sendMsgSendAggrKeyshareData({ value, fee: fullFee, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgSendAggrKeyshareData:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgSendAggrKeyshareData:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		...
		async MsgSendAggrKeyshareData({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.FairyringKeyshare.tx.msgSendAggrKeyshareData({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgSendAggrKeyshareData:Init Could not initialize signing client. Wallet is required.')
				} else{
					throw new Error('TxClient:MsgSendAggrKeyshareData:Create Could not create message: ' + e.message)
				}
			}
		},
