...
// func (k Keeper) SendSwapTx(ctx sdk.Context) error {
...
func (k Keeper) SendSwapTx(
	...
	if _, err := k.ChannelKeeper.SendPacket(ctx, channelCap, portID,
		sourceChannel, clienttypes.ZeroHeight(), uint64(ctx.BlockTime().UnixNano()+int64(20*time.Minute)), swapPacket); err != nil {
		return err
	}
