...
// EncryptedTxAllFromCondition returns all the encrypted TXs for a particular height
func (k Keeper) EncryptedTxAllFromCondition(c context.Context, req *types.QueryAllEncryptedTxFromconditionRequest) (*types.QueryAllEncryptedTxFromconditionResponse, error) {
	if req == nil {
		return nil, status.Error(codes.InvalidArgument, "invalid request")
	}

	ctx := sdk.UnwrapSDKContext(c)
	val := k.GetEncryptedTxAllFromCondition(ctx, req.TargetCondition)

	return &types.QueryAllEncryptedTxFromconditionResponse{EncryptedTxArray: val}, nil
}
