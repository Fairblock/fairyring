...
export { MsgRegisterHeight, MsgSubmitEncryptedTx, MsgSendCurrentHeight };

type sendMsgRegisterHeightParams = {
  value: MsgRegisterHeight,
  fee?: StdFee,
  memo?: string
};
...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		async sendMsgRegisterHeight({ value, fee, memo }: sendMsgRegisterHeightParams): Promise<DeliverTxResponse> {
			if (!signer) {
					throw new Error('TxClient:sendMsgRegisterHeight: Unable to sign Tx. Signer is not present.')
			}
			try {			
				const { address } = (await signer.getAccounts())[0]; 
				const signingClient = await SigningStargateClient.connectWithSigner(addr,signer,{registry, prefix});
				let msg = this.msgRegisterHeight({ value: MsgRegisterHeight.fromPartial(value) })
				return await signingClient.signAndBroadcast(address, [msg], fee ? fee : defaultFee, memo)
			} catch (e: any) {
				throw new Error('TxClient:sendMsgRegisterHeight: Could not broadcast Tx: '+ e.message)
			}
		},
		...
		msgRegisterHeight({ value }: msgRegisterHeightParams): EncodeObject {
			try {
				return { typeUrl: "/fairyring.fairblock.MsgRegisterHeight", value: MsgRegisterHeight.fromPartial( value ) }  
			} catch (e: any) {
				throw new Error('TxClient:MsgRegisterHeight: Could not create message: ' + e.message)
			}
		},
