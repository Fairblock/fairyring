...
func Fill(x interface{}) interface{} {
	...
	case reflect.Slice:
		for i := 0; i < v.Len(); i++ {
			obj := v.Index(i)
			objPt := reflect.NewAt(obj.Type(), unsafe.Pointer(obj.UnsafeAddr())).Interface() // #nosec G103
			objPt = Fill(objPt)
		...
		for i := 0; i < v.NumField(); i++ {
			...
			case reflect.Struct:
				...
				case coinsType:
					...
					f.Set(s)
				default:
					objPt := reflect.NewAt(f.Type(), unsafe.Pointer(f.UnsafeAddr())).Interface() // #nosec G103
					s := Fill(objPt)
